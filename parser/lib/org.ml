...
let parse_entry_to_org_mapping entry = 
  let name = List.nth entry 0 |> fun raw -> String.sub raw 0 (String.length raw) in
...
let parse content = 
  ...
  List.map (fun entry ->  split property_delim entry |> filter_properties_tag |> parse_entry_to_mapping )