...
let tree_to_stream tree = 
  from_tree (fun element -> match element with 
  | Text s -> `Text s
  | Element (name, atts, children) -> `Element(name,atts,children)) tree
...
let patch_entry element history_tbl mapping cur_path =
  ...
  if List.length location_list > 0 then
    ...
    if CCOpt.is_some matching then (
      ...
      Hashtbl.add deconstructed ("", "FILE") shortened ;
      Hashtbl.add deconstructed ("", "DIR") cur_path ;
      let new_location =
        Element (l_name, CCHashtbl.to_list deconstructed, l_children)
      in
      let element_atts = attibutes_to_hashtbl atts in
      Hashtbl.add element_atts ("", "ARTIST") record.Tracks.author ;
      Element
...
let patch_nml history mapping path dest =
  ...
  | Some tree -> patch_entries tree patch |> tree_to_stream |> write_to_file