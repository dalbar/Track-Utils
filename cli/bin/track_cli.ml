...
let track_cli recurisve shorten org path = 
  let rec loop cur_path dic_acc = 
    ...
    let record_map = if shorten then 
      ...
      let mapping_shortened_old = List.map (fun (key, record) -> (key,  Tracks.stringify_token_record record)) mapping_shortened_record in
      ...
      else List.map (fun record -> (Tracks.stringify_token_record record, record)) track_records in
    if org && List.length record_map > 0 then write_org_file record_map @@ concat_cur_path "db.org";
