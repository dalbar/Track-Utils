...
let reduce_version track_details special_symbol =
  ...
  let matches_once regexp =
    List.length (Re.matches regexp track_details.version) > 0
  in
  if matches_once mix then {track_details with version= new_version "Mix"}
  else if matches_once remix then
    {track_details with version= new_version "Remix"}
  else if matches_once version then
    {track_details with version= new_version "Version"}
  else track_details

type shortening_operation = Author | Title | Version

let shorten_track ?(special_symbol = special_symbol) ?(treshhold = 85)
    track_details =
  let cut f record = f record special_symbol in
  let cur_res =
    cut cut_features @@ cut cut_title_plus
    @@ cut cut_version_plus track_details
  in
...
let shorten_track_list ?(special_symbol = special_symbol) ?(treshhold = 85)
    record_list =
  let track_map = CCHashtbl.of_list [] in
  let has_duplicate name = Hashtbl.mem track_map name in
  let resolve_conflict record counter =
    {record with extension= string_of_int counter ^ "." ^ record.extension}
  in
  let shorten record =
